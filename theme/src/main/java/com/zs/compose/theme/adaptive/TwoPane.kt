...
// TODO: Consider removing the dialog parameter from TwoPane as it adds unnecessary complexity and
 ...
 * @param containerColor The background color of the layout. Defaults to `AppTheme.colors.background`.
 * @param contentColor The color of content displayed on the background. Defaults to `AppTheme.colors.onBackground`.
...
@Composable
@Deprecated("Use other TwoPane instead without dialog.")
private fun TwoPane(
    primary: @Composable () -> Unit,
    modifier: Modifier = Modifier,
    spacing: Dp = DEFAULT_SPACING,
    ...
    Layout(
        modifier = modifier
            ...
            .fillMaxSize(),
        measurePolicy = remember(strategy, fabPosition) {
            when (strategy) {
...
private data class TwoPaneHorizontalMeasurePolicy(
    ...
    override fun MeasureScope.measure(measurables: List<Measurable>, c: Constraints): MeasureResult {
        ...
        val remaining = maxOf(contentAllocatedWidth, (width - gapWidthPx /2) - detailsPlaceable.measuredWidth)
