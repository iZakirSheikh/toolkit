 ...
 *
 * Created by sheik on 02-02-2024.
 *
...
context(MeasureScope)
private val WindowInsets.values: Array<Int>
    get() {
        return arrayOf(
            getLeft(this@MeasureScope, layoutDirection),
            getTop(this@MeasureScope),
            getRight(this@MeasureScope, layoutDirection),
            getBottom(this@MeasureScope),
        )
    }
...
fun CollapsableTopBarLayout(
    ...
    Layout(
        ...
        measurePolicy = { measurables, incomming ->
            ...
            val (start, top, end, bottom) = insets.values
            ...
            val height = lerp(if (isLargeAppBar) topBarHeight else 0, topBarMaxHeight, fraction) + top + bottom
            ...
            val placeables = measurables.let {
                    list ->
                ...
                .map { measurable ->
                    when (measurable.layoutId) {
                        ...
                        APP_BAR_LAYOUT_ID_BACKGROUND -> measurable.measure(constraints.copy(maxHeight = height))
                        ...
                        else -> measurable.measure(constraints.copy(0, width - (start + end), 0, height - (top + bottom)))
                    }
                }
            layout(width, height) {
                ...
                placeables.forEach { placeable ->
                    when(placeable.layoutId){
                        APP_BAR_LAYOUT_ID_BACKGROUND -> placeable.place(fraction, IntSize(width, height))
                        APP_BAR_LAYOUT_ID_NAVIGATION_ICON -> {
                            val box = IntSize(placeable.width, topBarHeight)
                            val finalBox = if (!placeable.hasPlacementInfo) box else fullAvailableSpace
                            navigationWidth = placeable.width
                            placeable.place(fraction, box, finalBox, topStartOffset)
                        }
                        APP_BAR_LAYOUT_ID_ACTIONS -> {
                            val box = IntSize(placeable.width, topBarHeight)
                            val finalBox = if (!placeable.hasPlacementInfo) box else fullAvailableSpace
                            val offset = IntOffset(width - (end + placeable.width), top)
                            val finalOffset = if (!placeable.hasPlacementInfo) offset else topStartOffset
                            ...
                            placeable.place(fraction, box, finalBox, offset, finalOffset)
                        }
                        APP_BAR_LAYOUT_ID_TITLE, APP_BAR_LAYOUT_ID_COLLAPSABLE_TITLE -> {
                            val box = IntSize(titleSlotMaxWidth, topBarHeight)
                            val collapsable = placeable.layoutId == APP_BAR_LAYOUT_ID_COLLAPSABLE_TITLE
                            val finalBox = if (!collapsable || !isLargeAppBar) box else fullAvailableSpace
                        ...
                        else -> placeable.place(fraction, fullAvailableSpace, offset = topStartOffset)
                    }
                }
            }
        }
