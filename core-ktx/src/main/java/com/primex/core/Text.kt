...
/**
 * A simple function that returns the `Bundled` value inside this wrapper.
 * It is either the resource ID from which this Wrapper was created or the raw text `String`.
 *
 * @return The `Bundle` value inside this wrapper.
 */
val Text.raw: Any
    get() = when (this) {
        is HtmlResource -> id
 ...
 */
@JvmName("resolve2")
fun Resources.resolve(text: Text?): CharSequence? = if (text == null) null else resolve(text)
